...
这是一个 Codex Skill：它不按任务字数或单次调用价格粗略选模，而是按“正确完成任务的预期总成本”，在 **GPT-6 Astra** 与 **GPT-6 Sol** 之间分流；选定主模型后，还可把边界清楚、重复机械、判断负担低的批量工作委派给 **GPT-6 Luna**。
...
| 路由 | 适用任务 |
| --- | --- |
| GPT-6 Sol | 方法明确、验收标准清楚、错误局部、验证便宜 |
| GPT-6 Astra | 早期选择不确定、依赖深、错误暴露晚、返工代价高 |
| GPT-6 Luna 子智能体 | 范围明确、可独立完成、重复读取/提取/检查且解释负担低 |
...
如果你已经在桌面端把当前任务设为 6 Astra，Skill 会直接用 Astra 完成原本交给 Sol 的主智能体工作，包括后续的“继续”，不会要求切回 Sol。新任务重新按当时选用的模型处理；只在当前仍是 Sol 且任务确需 Astra 时建议切换。
...
启动器会先用 GPT-6 Sol 做一次轻量语义分类，再启动选中的模型。`-Model sol` 或 `-Model astra` 可显式覆盖分类；例如本次指定 Astra 时传 `-Model astra`。执行默认使用 `read-only` 沙盒；只有任务确实获准修改工作区时，才传入 `-Sandbox workspace-write`。
