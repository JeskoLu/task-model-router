...
description: 在开始执行新任务、工作范围扩大或出现实质返工时，按正确完成任务的总成本选择 GPT-6 Astra 或 GPT-5.6 Sol，并把值得委派的重复读取和简单检查交给 Luna；支持桌面选模建议和 CLI 自动执行。
---
...
两种主模型均遵循 [luna-delegation-policy.md](references/luna-delegation-policy.md)。有独立、范围清楚且值得委派的重复工作时，创建 `gpt-5.6-luna` 子智能体；没有合适工作则主模型直接执行。主模型负责计划、关键推理、整合和风险相称的验证。
